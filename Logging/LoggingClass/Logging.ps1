...
class Logging {
    ...
    Logging() {
        ...
        $this.LogFormat = "Level" + $this.Delimiter + "Date" # + $this.Delimiter + "message" # Default
        $this.DateFormat = "MMMM dd, yyyy hh:mm:ss" # Default
        $this.LogHeader = "" # Default -> header doesn't show
    } # end constructor
    ...
    [void] hidden CreateLogFile() {
        ...
        try {
            ...
            if ($this.FileMode -eq "Append") {
                ...
                if ($this.LogHeader -ne "") {
                    Add-Content `
                        -Path $this.Path `
                        -Value $this.LogHeader
                } # end if statement
            } # end if statement
    ...
    [void] hidden FormatMessage([String]$Level, [String]$Message) {
        ...
        # Add the message to message format
        if ("message" -notin $FormatArray) {
            if ($this.DebugMode) {
                Write-Host "Added `'message`' to Log format" -ForegroundColor Magenta
            } # end if statement

            $FormatArray += "message"
        } # end if statement

        # Create string to write to file or host
    ...
    <# GetLogHeader
        Returns the current Log Header as a String
    #>
    [String] GetLogHeader() {
    ...
    <# SetLogHeader
        - Takes in the string to be the header of the log file
        - Checks if it matches with the log format and warns the user if it does not
        - Sets the log header to the specified string
    #>
...
# Testing: Get rid of everything below
Write-Host "Start of Leveled LogWrite.ps1" -ForegroundColor Green
...
$Log.GetLogHeader()
$Log.SetLogHeader("Level:Date:Message")
# $Log.SetLogHeader("Level,Date,Message")
$Log.GetLogHeader()

# $Log.BasicConfig("DEBUG", "C:\Users\sutte\Documents\GitHub\Personal-PowerShell\Logging\Leveled LogWrite\test.log", "append")
$Log.BasicConfig("DEBUG", "test.log", "append")
# $Log.BasicConfig("DEBUG", "test", "append")
# $Log.BasicConfig("INFO", "test.log")
# $Log.BasicConfig("WARNING", "test.log", "w")
# $Log.BasicConfig("ERROR", "Write")
# $Log.BasicConfig("CRITICAL", "test.log", "a")
